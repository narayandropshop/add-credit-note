...
var matchIndexes = []int{0, 1}
var coloumnNames = []string{"Document Number", "Credit Adjustment"}
...
func ReadFile(dataInput utils.InputData, projectID string, resStatus *utils.RestStatus) *utils.RestStatus {
	...
	if os.Getenv("TEST") != "true" {
		...
		fmt.Println("in test")
		fileObj, err = excelize.OpenFile("/home/stingray/Downloads/RB_Credit_Adjustment.xlsx")
	}
...
func buildData(dataInput utils.InputData, rows [][]string, projectID string, resStatus *utils.RestStatus) {
	...
	for _, row := range rows[sheetStartIndex+1:] {
		...
		if len(row) > 1 {
			...
			newBill.CreditNote = utils.GetFloatFromStirng(row[1])
	...
	fmt.Println("total entries", count, biilIdsMap)
